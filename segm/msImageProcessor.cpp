...
msImageProcessor::~msImageProcessor( void )
{
	...
	if(mRegionList)					delete mRegionList;
	mRegionList = NULL;
...
void msImageProcessor::DefineBoundaries( void )
{
	...
	if(!(mRegionList	= new RegionList(regionCount, totalBoundaryCount, N)))
		ErrorHandler("msImageProcessor", "DefineBoundaries", "Not enough memory.");
	...
	for(i = 0; i < regionCount; i++)
	{
		mRegionList->AddRegion(i, boundaryCount[i], &boundaryBuffer[counter]);
		counter += boundaryCount[i];
	}
