...
RegionList::RegionList(int maxRegions_, int L_, int N_)
{
	...
	if(!(mRegionList = new REGION [maxRegions]))
		ErrorHandler("RegionList", "Not enough memory.", FATAL);
...
void RegionList::AddRegion(int label, int pointCount, int *indeces)
{
	...
	//freeRegion index
	mRegionList[freeRegion].label		= label;
	mRegionList[freeRegion].pointCount	= pointCount;
	mRegionList[freeRegion].region		= freeBlockLoc;
...
int	RegionList::GetLabel(int regionNum)
{
	...
	return mRegionList[regionNum].label;
}
...
int RegionList::GetRegionCount(int regionNum)
{
	...
	return mRegionList[regionNum].pointCount;
}
...
int *RegionList::GetRegionIndeces(int regionNum)
{
	...
	return &indexTable[mRegionList[regionNum].region];
}
